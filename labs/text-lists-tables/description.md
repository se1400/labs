...
14. Inside the <code>&lt;tbody&gt;</code>, add three rows with tuition data. Each row needs three cells (<code>&lt;td&gt;</code>). Visit Utah Tech's official tuition page and find the per-credit-hour rates: <a href="https://catalog.utahtech.edu/tuitionfees/" target="_blank">https://catalog.utahtech.edu/tuitionfees/</a>

    Look for rates for Undergraduate, Graduate, and Online programs (both resident and non-resident where applicable). The page has several tables—scroll through and do your best to find reasonable rates. Include the dollar sign ($) before each amount.
