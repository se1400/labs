...
3. In your CSS file, add `text-align: center` to the `header` rule so the university name and tagline are centered.

### Part 3: Special Characters

HTML character entities display special symbols. They start with `&` and end with `;`.

4. In the footer, find the paragraph with "Copyright Utah Tech University". Add a `<span>` element with `id="year"` between the copyright symbol and "Utah Tech University". Then add JavaScript just before the closing `</body>` tag to display the current year:
   ...
   Change "Copyright" to use the copyright symbol `&copy;` followed by the year span.

### Part 4: Text Formatting Tags

HTML provides tags for emphasizing text with semantic meaning.

5. In the Welcome section, find the sentence "We are committed to active learning and helping students succeed." Wrap "active learning" in `<strong>` tags to make it bold.

6. In the same sentence, wrap "succeed" in `<em>` tags to make it italic.

7. Find "20:1 student-faculty ratio" and add a superscript `<sup>` tag containing "1" immediately after "ratio" to create a footnote reference.

8. In the footer, wrap the address content in `<pre>` tags to preserve its formatting. You can remove the `<br>` tags since `<pre>` preserves line breaks automatically.

### Part 5: Unordered Lists

Unordered lists display items with bullet points using `<ul>` and `<li>` tags.

9. Replace the entire "Why Choose Utah Tech?" section with a new section titled "Our Colleges". Inside, create an unordered list with these five colleges:
...
10. Under the first college (College of Science, Engineering & Technology), add a nested unordered list containing three programs: Computer Science, Software Engineering, and Biology.

### Part 7: Ordered Lists

Ordered lists display numbered items using `<ol>` and `<li>` tags.

11. After the "Our Colleges" section, add a new section titled "How to Apply" with an ordered list containing three steps:
...
12. After the "How to Apply" section, add a new section titled "Tuition & Fees" with a table. The table should have:
...
- **Special Characters** - Using character entities like `&copy;`
