...
12. The three anchor elements should have an `img` element with an appropriate `alt` attribute and a `src` attribute set to a valid image as their content. You can use `labs/assets/colosseo.jpg`, `labs/assets/alps.jpg`, and `labs/assets/sea.jpg` if you would like.
13. All your five anchor elements should have an `href` attribute with the value of `https://se1400.github.io/labs` and a `target` attribute with the value of `_blank`.
